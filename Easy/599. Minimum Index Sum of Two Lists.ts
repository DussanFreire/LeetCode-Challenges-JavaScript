...
function findRestaurant(list1: string[], list2: string[]): string[] {
    const list1Indices = new Map<string, number>();
    const result: string[] = [];
    let minIndexSum = Infinity;

    list1.forEach((restaurant, index) => {
        list1Indices.set(restaurant, index);
    });

    for (let j = 0; j < list2.length; j++) {
        const restaurant = list2[j];

        if (list1Indices.has(restaurant)) {
            const i = list1Indices.get(restaurant)!;
            const currentIndexSum = i + j;
