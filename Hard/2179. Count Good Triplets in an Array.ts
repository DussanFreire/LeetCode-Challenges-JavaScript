...
function goodTriplets(nums1: number[], nums2: number[]): number {
    ...
    const rightLarger = new Array(n).fill(0);
    ...
    for (let i = n - 1; i >= 0; i--) {
        const greaterCount = rightTree.query(n - 1) - rightTree.query(transformed[i]);
        
        rightLarger[i] = greaterCount;
        rightTree.update(transformed[i], 1);
    }

    for (let i = 0; i < n; i++) {
        total += leftSmaller[i] * rightLarger[i];
    }
