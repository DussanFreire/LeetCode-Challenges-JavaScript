...
function snakesAndLadders(board: number[][]): number {
    ...
    function getCoordinates(square: number): [number, number] {
        const r = n - 1 - Math.floor((square - 1) / n);
        let c = (square - 1) % n;
        if ((n - 1 - r) % 2 === 1) {
            c = n - 1 - c;
        }
    ...
    const visited = new Array(totalSquares + 1).fill(false);
    ...
    while (queue.length > 0) {
        ...
        if (curr === totalSquares) return moves;

        for (let next = curr + 1; next <= Math.min(curr + 6, totalSquares); next++) {
